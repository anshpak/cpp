...
public:
	...
	~Tree();
	void push(node<my_type>*& vertex, my_type data); // вставка элемента в дерево
	void show(node<my_type>* vertex); // вывод дерева на экран
	node<my_type>* find(node<my_type>* vertex, my_type key); // поиск по ключу
...
template <class my_type> void Tree<my_type>::push(node<my_type>*& vertex, my_type data) {
	if (vertex == 0) {
...
template <class my_type> node<my_type>* Tree<my_type>::find(node<my_type>* vertex, my_type key) {
	if (vertex == 0) {
		return 0;
	}
